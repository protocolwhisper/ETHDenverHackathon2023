...
*/
`;function E(n){let{data:r,template:t}=n,e={strict:!0,noEscape:!0},i=ot(t,e),s=ot(pt,e);return i({...r,header:s(Ft)}).replace(/[\n]{3,}/gm,`
...
{{/each}}
`;function Et(n){let r=n.abis.map(({name:e})=>e);return E({template:At,data:{abiCapitalizedNames:r}})}var J=class{constructor(r){let{abiFiles:t,outputDir:e}=r;this.files=[],this.outputDir=e,this.abiFiles=t,this.abis=this.abiFiles.map(i=>new B({filepath:i.path,rawContents:JSON.parse(i.contents),outputDir:e})),this.assembleAllFiles()}assembleAllFiles(){let r=this.abis.find(e=>e.commonTypesInUse.length>0);this.abis.forEach(e=>{let i={path:e.dtsFilepath,contents:e.getDtsDeclaration()},s={path:e.factoryFilepath,contents:e.getFactoryDeclaration()};this.files.push(i),this.files.push(s)});let t={path:`${this.outputDir}/index.ts`,contents:Et({abis:this.abis})};if(this.files.push(t),r){let i={path:Gt(this.outputDir,"common.d.ts"),contents:ht()};this.files.push(i)}}};function wt(n){let{cwd:r,inputs:t,output:e,silent:i,filepaths:s}=n,u=Ot(r),{log:a}=console;i&&(a=()=>({}));let l=[];if(!(s!=null&&s.length)&&(t==null?void 0:t.length))l=t.flatMap(m=>Ht(m,{cwd:r}));else if(s!=null&&s.length)l=s;else throw new Error("You need to inform at least one parameter: `input` or `filepaths`");let y=l.map(m=>({path:m,contents:_t(m,"utf-8")})),b=new J({outputDir:e,abiFiles:y});a(`Generating files..
`),Xt.sync(`${e}/factories`),b.files.forEach(m=>{Ut.sync(m.path),Mt(m.path,m.contents);let c=new RegExp(`^.+${u}/`,"m");a(` - ${m.path.replace(c,"")}`)}),a(`
Done.\u26A1`)}function Pt(n){let r=process.cwd(),{inputs:t,output:e,silent:i}=n;wt({cwd:r,inputs:t,output:e,silent:!!i})}function Vt(n){n.requiredOption("-i, --inputs <path|glob...>","input paths/globals to your abi json files").requiredOption("-o, --output <dir>","directory path for generated files").option("-s, --silent","omit output messages").action(Pt)}function oi(n){let r=new kt,{argv:t,programName:e}=n;r.name(e),r.version(Dt.FUELS),r.usage("-i ../out/*-abi.json -o ./generated/"),Vt(r),r.parse(t)}export{Vt as configureCliOptions,oi as run,Pt as runCliAction};
