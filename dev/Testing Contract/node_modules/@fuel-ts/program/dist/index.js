"use strict";var N=Object.defineProperty;var o0=Object.getOwnPropertyDescriptor;var s0=Object.getOwnPropertyNames;var i0=Object.prototype.hasOwnProperty;var c0=(r,t)=>{for(var e in t)N(r,e,{get:t[e],enumerable:!0})},u0=(r,t,e,n)=>{if(t&&typeof t=="object"||typeof t=="function")for(let a of s0(t))!i0.call(r,a)&&a!==e&&N(r,a,{get:()=>t[a],enumerable:!(n=o0(t,a))||n.enumerable});return r};var p0=r=>u0(N({},"__esModule",{value:!0}),r);var R0={};c0(R0,{Contract:()=>L,FunctionInvocationResult:()=>R,FunctionInvocationScope:()=>I,InvocationResult:()=>S,MultiCallInvocationScope:()=>w,ScriptRequest:()=>b,ScriptResultDecoderError:()=>x,assert:()=>T,getDocs:()=>E});module.exports=p0(R0);var V=["Success","Revert","OutOfGas","TransactionValidity","MemoryOverflow","ArithmeticOverflow","ContractNotFound","MemoryOwnership","NotEnoughBalance","ExpectedInternalContext","AssetIdNotFound","InputNotFound","OutputNotFound","WitnessNotFound","TransactionMaturity","InvalidMetadataIdentifier","MalformedCallStructure","ReservedRegisterNotWritable","ErrorFlag","InvalidImmediateValue","ExpectedCoinInput","MaxMemoryAccess","MemoryWriteOverlap","ContractNotInInputs","InternalBalanceOverflow","ContractMaxSize","ExpectedUnallocatedStack","MaxStaticContractsReached","TransferAmountCannotBeZero","ExpectedOutputVariable","ExpectedParentInternalContext","IllegalJump","NonZeroMessageOutputRecipient","ZeroedMessageOutputRecipient"],P="https://docs.rs/fuel-asm/latest/fuel_asm/enum.PanicReason.html";var l0=r=>V.includes(r)?r:"unknown",E=r=>{if((r==null?void 0:r.type)==="failure"){let t=l0(r.reason);return{doc:t!=="unknown"?`${P}#variant.${t}`:P,reason:t}}return{doc:P,reason:"unknown"}};function T(r,t){if(!r)throw new Error(t)}var Q=require("@fuel-ts/constants"),M=require("@fuel-ts/transactions");var U=(r,t)=>typeof t=="bigint"?t.toString():t,d0=(r,t)=>`${r===Q.ZeroBytes32?"script":r}: ${t}`,x=class extends Error{constructor(e,n,a){let o=JSON.stringify(E(e.status),null,2),i=e.receipts.filter(u=>u.type===M.ReceiptType.Revert),s=i.length?`Reverts:
${i.map(({id:u,...y})=>d0(u,`${y.val} ${JSON.stringify(y,U)}`)).join(`
`)}`:null,p=a.length?`Logs:
${JSON.stringify(a,null,2)}`:null,c=`Receipts:
${JSON.stringify(e.receipts.map(({type:u,...y})=>({type:M.ReceiptType[u],...y})),U,2)}`;super(`${n}

${o}

${s?`${s}

`:""}${p?`${p}

`:""}${c}`);this.logs=a}};var t0=require("@fuel-ts/providers");var f=require("@fuel-ts/math"),O=require("@fuel-ts/providers"),F=require("@fuel-ts/transactions");var v=require("@ethersproject/bytes"),h=require("@fuel-ts/abi-coder"),g=require("@fuel-ts/math"),z=require("@fuel-ts/transactions");var B=[{type:"function",inputs:[{name:"script_data",type:"struct ScriptData",components:[{name:"calls",type:"[enum Option; 5]",components:[{name:"__array_element",type:"enum Option",components:[{name:"None",type:"()",components:[],typeArguments:null},{name:"Some",type:"struct MulticallCall",components:[{name:"contract_id",type:"struct ContractId",components:[{name:"value",type:"b256",components:null,typeArguments:null}],typeArguments:null},{name:"fn_selector",type:"u64",components:null,typeArguments:null},{name:"fn_arg",type:"enum CallValue",components:[{name:"Value",type:"u64",components:null,typeArguments:null},{name:"Data",type:"(u64, u64)",components:[{name:"__tuple_element",type:"u64",components:null,typeArguments:null},{name:"__tuple_element",type:"u64",components:null,typeArguments:null}],typeArguments:null}],typeArguments:null},{name:"parameters",type:"struct CallParameters",components:[{name:"amount",type:"enum Option",components:[{name:"None",type:"()",components:[],typeArguments:null},{name:"Some",type:"u64",components:null,typeArguments:null}],typeArguments:[{name:"T",type:"u64",components:null,typeArguments:null}]},{name:"asset_id",type:"enum Option",components:[{name:"None",type:"()",components:[],typeArguments:null},{name:"Some",type:"struct ContractId",components:[{name:"value",type:"b256",components:null,typeArguments:null}],typeArguments:null}],typeArguments:[{name:"T",type:"struct ContractId",components:[{name:"value",type:"b256",components:null,typeArguments:null}],typeArguments:null}]},{name:"gas",type:"enum Option",components:[{name:"None",type:"()",components:[],typeArguments:null},{name:"Some",type:"u64",components:null,typeArguments:null}],typeArguments:[{name:"T",type:"u64",components:null,typeArguments:null}]}],typeArguments:null}],typeArguments:null}],typeArguments:[{name:"T",type:"struct MulticallCall",components:[{name:"contract_id",type:"struct ContractId",components:[{name:"value",type:"b256",components:null,typeArguments:null}],typeArguments:null},{name:"fn_selector",type:"u64",components:null,typeArguments:null},{name:"fn_arg",type:"enum CallValue",components:[{name:"Value",type:"u64",components:null,typeArguments:null},{name:"Data",type:"(u64, u64)",components:[{name:"__tuple_element",type:"u64",components:null,typeArguments:null},{name:"__tuple_element",type:"u64",components:null,typeArguments:null}],typeArguments:null}],typeArguments:null},{name:"parameters",type:"struct CallParameters",components:[{name:"amount",type:"enum Option",components:[{name:"None",type:"()",components:[],typeArguments:null},{name:"Some",type:"u64",components:null,typeArguments:null}],typeArguments:[{name:"T",type:"u64",components:null,typeArguments:null}]},{name:"asset_id",type:"enum Option",components:[{name:"None",type:"()",components:[],typeArguments:null},{name:"Some",type:"struct ContractId",components:[{name:"value",type:"b256",components:null,typeArguments:null}],typeArguments:null}],typeArguments:[{name:"T",type:"struct ContractId",components:[{name:"value",type:"b256",components:null,typeArguments:null}],typeArguments:null}]},{name:"gas",type:"enum Option",components:[{name:"None",type:"()",components:[],typeArguments:null},{name:"Some",type:"u64",components:null,typeArguments:null}],typeArguments:[{name:"T",type:"u64",components:null,typeArguments:null}]}],typeArguments:null}],typeArguments:null}]}],typeArguments:null}],typeArguments:null}],name:"main",outputs:[{name:"",type:"struct ScriptReturn",components:[{name:"call_returns",type:"[enum Option; 5]",components:[{name:"__array_element",type:"enum Option",components:[{name:"None",type:"()",components:[],typeArguments:null},{name:"Some",type:"enum CallValue",components:[{name:"Value",type:"u64",components:null,typeArguments:null},{name:"Data",type:"(u64, u64)",components:[{name:"__tuple_element",type:"u64",components:null,typeArguments:null},{name:"__tuple_element",type:"u64",components:null,typeArguments:null}],typeArguments:null}],typeArguments:null}],typeArguments:[{name:"T",type:"enum CallValue",components:[{name:"Value",type:"u64",components:null,typeArguments:null},{name:"Data",type:"(u64, u64)",components:[{name:"__tuple_element",type:"u64",components:null,typeArguments:null},{name:"__tuple_element",type:"u64",components:null,typeArguments:null}],typeArguments:null}],typeArguments:null}]}],typeArguments:null}],typeArguments:null}]}];var W="";var Z=require("@ethersproject/bytes"),D=require("@ethersproject/logger"),m=require("@fuel-ts/abi-coder"),d=require("@fuel-ts/transactions"),j=require("@fuel-ts/versions");var J=new D.Logger(j.versions.FUELS);function f0(r){let t=[...r.receipts],e=t.pop();if(!e)throw new Error("Expected scriptResultReceipt");if(e.type!==d.ReceiptType.ScriptResult)throw new Error(`Invalid scriptResultReceipt type: ${e.type}`);let n=t.pop();if(!n)throw new Error("Expected returnReceipt");if(n.type!==d.ReceiptType.Return&&n.type!==d.ReceiptType.ReturnData&&n.type!==d.ReceiptType.Revert)throw new Error(`Invalid returnReceipt type: ${n.type}`);return{code:e.result,gasUsed:e.gasUsed,receipts:t,scriptResultReceipt:e,returnReceipt:n,callResult:r}}function X(r,t,e=[]){try{let n=f0(r);return t(n)}catch(n){throw new x(r,n.message,e)}}function G(r,t,e){return X(r,n=>{n.returnReceipt.type===d.ReceiptType.Revert&&J.throwError("Script Reverted",D.Logger.errors.CALL_EXCEPTION,e),n.returnReceipt.type!==d.ReceiptType.Return&&n.returnReceipt.type!==d.ReceiptType.ReturnData&&J.throwError(`Script Return Type [${n.returnReceipt.type}] Invalid`,D.Logger.errors.CALL_EXCEPTION,{logs:e,receipt:n.returnReceipt});let a;return n.returnReceipt.type===d.ReceiptType.Return&&(a=n.returnReceipt.val),n.returnReceipt.type===d.ReceiptType.ReturnData&&(a=t.program.interface.decodeFunctionResult(t.func,n.returnReceipt.data)[0]),a},e)}var b=class{constructor(t,e,n){this.bytes=(0,Z.arrayify)(t),this.scriptDataEncoder=e,this.scriptResultDecoder=n}getScriptDataOffset(){return m.VM_TX_MEMORY+m.TRANSACTION_SCRIPT_FIXED_SIZE+new d.ByteArrayCoder(this.bytes.length).encodedLength}getArgOffset(){return this.getScriptDataOffset()+m.ASSET_ID_LEN+m.WORD_SIZE+m.CONTRACT_ID_LEN+m.WORD_SIZE+m.WORD_SIZE}encodeScriptData(t){return this.scriptDataEncoder(t)}decodeCallResult(t,e=[]){return X(t,this.scriptResultDecoder,e)}};var _=new b(W,r=>{let t=B[0].inputs,e=new h.AbiCoder().getCoder(t[0]),n=e.coders.calls.length;if(r.length>n)throw new Error(`At most ${n} calls are supported`);let a=new Uint8Array,o=[];for(let p=0;p<n;p+=1){let c=r[p],u;if(c){let y=(0,v.arrayify)(c.data),n0=y.slice(0,8),r0=y.slice(8,16).some(a0=>a0===1),k=y.slice(16),q;r0?(q={Data:[a.length,k.length]},a=(0,v.concat)([a,k])):q={Value:new h.U64Coder().decode(k,0)[0]},u={contract_id:{value:c.contractId},fn_selector:new h.U64Coder().decode(n0,0)[0],fn_arg:q,parameters:{amount:c.amount?(0,g.bn)(c.amount):void 0,asset_id:c.assetId?{value:c.assetId}:void 0,gas:c.gas?(0,g.bn)(c.gas):void 0}}}else u=void 0;o.push(u)}let i={calls:o},s=e.encode(i);return(0,v.concat)([s,a])},r=>{if((0,g.toNumber)(r.code)!==0)throw new Error(`Script returned non-zero result: ${r.code}`);if(r.returnReceipt.type!==z.ReceiptType.ReturnData)throw new Error("Expected returnReceipt to be a ReturnDataReceipt");let t=(0,v.arrayify)(r.returnReceipt.data),e=B[0].outputs,n=new h.AbiCoder().getCoder(e[0]),[a,o]=n.decode(t,0),i=t.slice(o),s=[];return a.call_returns.forEach((p,c)=>{if(p)if(p.Data){let[u,y]=p.Data;s[c]=i.slice((0,g.toNumber)(u),(0,g.toNumber)(u)+(0,g.toNumber)(y))}else s[c]=new h.U64Coder().encode(p.Value)}),s});var Y=require("@fuel-ts/math"),H=require("@fuel-ts/providers"),K=require("@fuel-ts/transactions");function y0(r){let t=r.receipts.find(e=>e.type===K.ReceiptType.ScriptResult);return(t==null?void 0:t.gasUsed)||(0,Y.bn)(0)}var S=class{constructor(t,e,n){this.functionScopes=Array.isArray(t)?t:[t],this.isMultiCall=n,this.value=this.getDecodedValue(e),this.gasUsed=y0(e)}getFirstCallConfig(){if(!!this.functionScopes[0])return this.functionScopes[0].getCallConfig()}getDecodedValue(t){let e=this.getDecodedLogs(t.receipts),n=this.getFirstCallConfig();if(this.functionScopes.length===1&&n&&"bytes"in n.program)return G(t,n,e);let o=_.decodeCallResult(t,e).map((i,s)=>{var u;let{program:p,func:c}=this.functionScopes[s].getCallConfig();return(u=p.interface.decodeFunctionResult(c,i))==null?void 0:u[0]});return this.isMultiCall?o:o==null?void 0:o[0]}getDecodedLogs(t){let e=this.getFirstCallConfig();if(!e)return[];let{program:n}=e;return(0,H.getDecodedLogs)(t,n.interface)}},R=class extends S{constructor(e,n,a,o,i){super(e,a,i);this.transactionResponse=n,this.transactionResult=a,this.transactionId=this.transactionResponse.id,this.program=o,this.logs=this.getDecodedLogs(a.receipts)}static async build(e,n,a,o){let i=await n.waitForResult();return new R(e,n,i,o,a)}},A=class extends S{constructor(e,n,a){super(e,n,a);this.callResult=n}static async build(e,n,a){return new A(e,n,a)}};function g0(r){let{program:t,args:e,forward:n,func:a,callParameters:o,bytesOffset:i}=r.getCallConfig(),s=t.interface.encodeFunctionData(a,e,_.getScriptDataOffset()+i);return{contractId:t.id,data:s,assetId:n==null?void 0:n.assetId,amount:n==null?void 0:n.amount,gas:o==null?void 0:o.gasLimit}}var C=class{constructor(t,e){this.functionInvocationScopes=[];this.requiredCoins=[];this.isMultiCall=!1;this.program=t,this.isMultiCall=e,this.transactionRequest=new O.ScriptTransactionRequest({gasLimit:F.MAX_GAS_PER_TX})}get calls(){return this.functionInvocationScopes.map(t=>g0(t))}static getCallOptions(t){return{fundTransaction:!0,...t}}updateScriptRequest(){let t=this.calls;t.forEach(e=>{this.transactionRequest.addContract(e.contractId)}),this.transactionRequest.setScript(_,t)}getRequiredCoins(){return this.calls.map(e=>({assetId:String(e.assetId),amount:(0,f.bn)(e.amount||0)})).concat(this.transactionRequest.calculateFee()).filter(({assetId:e,amount:n})=>e&&!(0,f.bn)(n).isZero())}updateRequiredCoins(){let t=this.getRequiredCoins(),e=(n,{assetId:a,amount:o})=>{var s;let i=((s=n.get(a))==null?void 0:s.amount)||(0,f.bn)(0);return n.set(a,{assetId:String(a),amount:i.add(o)})};this.requiredCoins=Array.from(t.reduce(e,new Map).values())}addCall(t){return this.addCalls([t]),this}addCalls(t){return this.functionInvocationScopes.push(...t),this.updateScriptRequest(),this.updateRequiredCoins(),this}async prepareTransaction(t){this.updateScriptRequest(),this.updateRequiredCoins(),this.checkGasLimitTotal(),C.getCallOptions(t).fundTransaction&&this.program.account&&await this.fundWithRequiredCoins()}checkGasLimitTotal(){if(this.calls.reduce((e,n)=>e.add(n.gas||0),(0,f.bn)(0)).gt(this.transactionRequest.gasLimit))throw new Error("Transaction gasLimit can't be lower than the sum of the forwarded gas of each call")}async getTransactionCost(t){var o;let e=((o=this.program.account)==null?void 0:o.provider)||this.program.provider;T(e,"Wallet or Provider is required!"),await this.prepareTransaction(t);let n=(0,O.transactionRequestify)(this.transactionRequest);return n.gasPrice=(0,f.bn)((0,f.toNumber)(n.gasPrice)||(0,f.toNumber)((t==null?void 0:t.gasPrice)||0)),await e.getTransactionCost(n,t==null?void 0:t.tolerance)}async fundWithRequiredCoins(){var e;this.transactionRequest.inputs=this.transactionRequest.inputs.filter(n=>n.type!==F.InputType.Coin);let t=await((e=this.program.account)==null?void 0:e.getResourcesToSpend(this.requiredCoins));return this.transactionRequest.addResources(t||[]),this}txParams(t){var n;this.txParameters=t;let e=this.transactionRequest;return e.gasLimit=(0,f.bn)(t.gasLimit||e.gasLimit),e.gasPrice=(0,f.bn)(t.gasPrice||e.gasPrice),e.addVariableOutputs(((n=this.txParameters)==null?void 0:n.variableOutputs)||0),this}addContracts(t){return t.forEach(e=>{this.transactionRequest.addContract(e.id),this.program.interface.updateExternalLoggedTypes(e.id.toB256(),[...e.interface.loggedTypes])}),this}async getTransactionRequest(t){return await this.prepareTransaction(t),this.transactionRequest}async call(t){T(this.program.account,"Wallet is required!");let e=await this.getTransactionRequest(t),n=await this.program.account.sendTransaction(e);return R.build(this.functionInvocationScopes,n,this.isMultiCall,this.program)}async simulate(t){T(this.program.account,"Wallet is required!");let e=await this.getTransactionRequest(t),n=await this.program.account.simulateTransaction(e);return A.build(this.functionInvocationScopes,n,this.isMultiCall)}async dryRun(t){var s;let e=((s=this.program.account)==null?void 0:s.provider)||this.program.provider;T(e,"Wallet or Provider is required!");let n=await this.getTransactionRequest(t),a=(0,O.transactionRequestify)(n),o=await e.call(a,{utxoValidation:!1});return await A.build(this.functionInvocationScopes,o,this.isMultiCall)}async get(t){return this.dryRun({fundTransaction:!1,...t})}};var I=class extends C{constructor(e,n,a){super(e,!1);this.func=n,this.args=a||[],this.setArguments(...a),super.addCall(this)}getCallConfig(){return{func:this.func,program:this.program,callParameters:this.callParameters,txParameters:this.txParameters,forward:this.forward,args:this.args,bytesOffset:this.transactionRequest.bytesOffset||0}}setArguments(...e){return this.args=e||[],this.updateScriptRequest(),this}callParams(e){return this.callParameters=e,e!=null&&e.forward&&(this.forward=(0,t0.coinQuantityfy)(e.forward)),this.setArguments(...this.args),this.updateRequiredCoins(),this}};var w=class extends C{constructor(t,e){super(t,!0),this.addCalls(e)}addCall(t){return super.addCalls([t])}addCalls(t){return super.addCalls(t)}};var $=require("@fuel-ts/abi-coder"),e0=require("@fuel-ts/address");var L=class{constructor(t,e,n){this.functions={};this.interface=e instanceof $.Interface?e:new $.Interface(e),this.id=e0.Address.fromAddressOrString(t),n&&"provider"in n?(this.provider=n.provider,this.account=n):(this.provider=n,this.account=null),Object.keys(this.interface.functions).forEach(a=>{let o=this.interface.getFunction(a);Object.defineProperty(this.functions,o.name,{value:this.buildFunction(o),writable:!1})})}buildFunction(t){return(...e)=>new I(this,t,e)}multiCall(t){return new w(this,t)}getBalance(t){if(!this.provider)throw new Error("Contract instance has no provider.");return this.provider.getContractBalance(this.id,t)}};0&&(module.exports={Contract,FunctionInvocationResult,FunctionInvocationScope,InvocationResult,MultiCallInvocationScope,ScriptRequest,ScriptResultDecoderError,assert,getDocs});
//# sourceMappingURL=index.js.map